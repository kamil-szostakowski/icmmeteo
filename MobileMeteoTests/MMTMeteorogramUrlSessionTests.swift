...
class MMTMeteorogramUrlSessionTests: XCTestCase
{
    ...
    {
        let data = validContent.data(using: String.Encoding.windowsCP1250)!
        XCTAssertEqual(validContent, session.htmlStringFromResponseData(data))
    }
    
    func testContentParseWithInvalidEncoding()
    {
        let data = validContent.data(using: String.Encoding.utf8)!        
    ...
    fileprivate func contentWithDateString(_ dateString: String) -> String
    {
